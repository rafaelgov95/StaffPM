...
import br.ufms.staff.app.StaffUpdate;
import br.ufms.staff.app.DialogPolicial;
...
public class StaffController implements Initializable {
    ...
    private void update(ActionEvent event) {
        new StaffUpdate().start(new Stage());
    }
    ...
    private void selectRow(MouseEvent event) {
        if (event.getClickCount() == 2) {
            PolicialPojo pojo = table.getSelectionModel().getSelectedItem();
            StaffUpdateViewController.id = pojo.getId();
            System.out.println(StaffUpdateViewController.id);
            new StaffUpdate().start(new Stage());
        }
    }
